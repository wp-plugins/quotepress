...
class wpCSL_plugin__quotepress {
    ...
    function create_notifications($class = 'none') {
        switch ($class) {
            ...
            case 'wpCSL_notifications__quotepress':
            case 'default':
            default:
                $this->notifications = 
                    new wpCSL_notifications__quotepress($this->notifications_config);
        }
    }
    ...
    function create_products($class = 'none') {
        switch ($class) {
            ...
            case 'wpCSL_products__quotepress':
            case 'default':
            default:
                $this->products = new wpCSL_products__quotepress($this->products_config);
    ...
    function create_settings($class = 'none') {
        switch ($class) {
            ...
            case 'wpCSL_settings__quotepress':
            case 'default':
            default:
                $this->settings = new wpCSL_settings__quotepress($this->settings_config);
    ...
    function create_themes($class = 'none') {
        switch ($class) {
            ...
            case 'default':
            default:
                $this->themes = new wpCSL_themes__quotepress($this->themes_config);
    ...
    function create_license($class = 'none') {
        switch ($class) {
            ...
            case 'wpCSL_license__quotepress':
            case 'default':
            default:
                if ($this->has_packages || !$this->no_license) {
                    $this->license = new wpCSL_license__quotepress($this->license_config);
                }
    ...
    function create_cache($class = 'none') {
        switch ($class) {
            ...
            case 'wpCSL_cache__quotepress':
            case 'default':
            default:
                $this->cache = new wpCSL_cache__quotepress($this->cache_config);
    ...
    function add_wp_actions() {
        ...
        //
            ...
            if (($this->debugging) && ($this->driver_type != 'none')) {
                print __('DEBUG: No driver found.',WPCSL__quotepress__VERSION);
            }
        }
    }
    ...
    function add_meta_links($links, $file) {
        ...
        if ($file == $this->basefile) {
            if (isset($this->support_url)) {
                $links[] = '<a href="'.$this->support_url.'" title="'.__('Support',WPCSL__quotepress__VERSION) . '">'.
                            __('Support',WPCSL__quotepress__VERSION) . '</a>';
            }
            if (isset($this->purchase_url)) {
                $links[] = '<a href="'.$this->purchase_url.'" title="'.__('Purchase',WPCSL__quotepress__VERSION) . '">'.
                            __('Buy Now',WPCSL__quotepress__VERSION) . '</a>';
            }
            $links[] = '<a href="options-general.php?page='.$this->prefix.'-options" title="'.
                            __('Settings',WPCSL__quotepress__VERSION) . '">'.__('Settings',WPCSL__quotepress__VERSION) . '</a>';
        }
        return $links;
    }
    ...
    function load_driver() {
        ...
        //
            ...
            //
                ...
                if ($this->debugging) {
                    print __('DEBUG: could not locate driver:',WPCSL__quotepress__VERSION) . 
    ...
    function add_display_settings() {      
        $this->settings->add_section(array(
                'name' => __('Display Settings',WPCSL__quotepress__VERSION),
                'description' => '',
                ...
                if (exec('locale -a', $locales)) {
                    ...
                    $this->settings->add_item(
                        ...
                        __('Sets the locale for PHP program processing, affects time and currency processing. '.
                            'If you change this, save settings and then select money format.',WPCSL__quotepress__VERSION),
                        $locale_custom
                    );
                }
            } else {
                    $this->settings->add_item(
                        ...
                        null, 
                        false, 
                        __('Your PHP settings have disabled exec(), your locale list cannot be determined.',WPCSL__quotepress__VERSION),
                        '&nbsp;'
                    );
            }
        }            
        ...
        if  (
            ...
            (function_exists('money_format')) 
                ...
                $this->settings->add_item(
                    ...
                    'list', 
                    false, 
                    __('This is based on your current locale, which is set to ',WPCSL__quotepress__VERSION).
       ...
       if (isset($this->rate_url)){
        	...
        	$this->settings->add_item(
        		...
        		'checkbox', 
        		false, 
        		__('This will disable the notification asking you to rate our product.',WPCSL__quotepress__VERSION)
        		);
    ...
    function shortcode_show_items($atts, $content = NULL) {
        if ( $this->ok_to_show() ) {
            ...
            if ($this->debugging) {
                if (is_array($atts)) {
                    print __('DEBUG: Shortcode called with attributes:',WPCSL__quotepress__VERSION) . "<br/>\n";
                    foreach ($atts as $name=>$value) {
                        print $name.':'.$value."<br/>\n";
                    }
                } else {
                    print __('DEBUG: Shortcode called with no attributes.',WPCSL__quotepress__VERSION) . "<br/>\n";
                }
            }            
        ...
        // Not OK TO Show
            ...
            if ($this->debugging) {
                $content = __('DEBUG: Not OK To Show',WPCSL__quotepress__VERSION);
            }
        }
        return $content;
    }
    ...
    function render_object_list($atts) {
        ...
        //
            ...
            $content= __('No products found',WPCSL__quotepress__VERSION);
        }
